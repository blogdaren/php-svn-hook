...
class PreCommitManager {
  ...
  public function getChecksToProcess(){
    ...
    /*
     *$first_checker = '';
     *foreach($checks as $k => $checker)
     *{
     *    if(strtolower($checker) == 'skipdir') 
     *    {
     *        $first_checker = $checker;
     *        unset($checks[$k]);
     *    }
     *}
     *!empty($first_checker) && array_unshift($checks, $first_checker);
     */
    //置顶SkipCheck检查器
  ...
  public function getErrorMsg(){
    ...
    {
      $detail .= '****** '.$check->getTitle().': '.$check->renderErrorSummary()."******\n";
      $detail .= $check->renderErrorDetail()."\n".$check->renderInstructions()."\n";
    }
